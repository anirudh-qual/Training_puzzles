...
im = ax.imshow(
    target_ratio_grid, cmap=tc.tol_cmap("BuRd"), vmin=0.5, vmax=1.5, origin="lower"
)  # , aspect=0.8)
...
for col, l in enumerate(layers):
    ...
    for row, h in enumerate(hiddens):
        ...
        text = ax.text(
            col, row, result, ha="center", va="center", color="k", size=8, weight=weight
        )
        if row == best_row:
            attn_size = int(step_rounder(hiddens[row] / layers[col]))
            params = human_format(
                transformer_num_parameters(hiddens[row], layers[col]), precision=0
            )
            ratio = hiddens[row] / layers[col]
            print(
                f"{params},{hiddens[row]},{hiddens[row]*4},8192,{layers[col]},{attn_size},{layers[col]},3072,{ratio}"
            )

exit(0)
ax.spines[:].set_visible(False)
ax.set_xticks(np.arange(params_grid.shape[1] + 1) - 0.5, minor=True)
ax.set_yticks(np.arange(params_grid.shape[0] + 1) - 0.5, minor=True)
