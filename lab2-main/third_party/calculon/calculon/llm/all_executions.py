...
class AllExecutions(calculon.CommandLine):
    ...
    def create_parser(subparser):
        ...
        sp.add_argument(
            "output",
            type=str,
            help="File path to the output file" " ('*.csv', '*.csv.gz')",
        )
        sp.add_argument(
            "-c",
            "--cpus",
    ...
    @staticmethod
    def all_executions(
        app, syst, num_procs, max_batch_size, datatype, fused_activation
    ):
                    ...
                    for activation_recompute in ["full", "attn_only", "none"]:
                        for optimizer_sharding in pick(dp > 1, [True, False], [False]):
                            for tensor_par_comm_type in ["ar", "p2p_rs_ag", "rs_ag"]:
                                can_redo = Llm.can_redo_ag(
                                    tensor_par_comm_type, activation_recompute
                                )
                                for seq_par_ag_redo in pick(
                                    can_redo, [True, False], [False]
                                ):
                                    for data_par_overlap in pick(
                                        dp > 1, [True, False], [False]
                                    ):
                                        for tensor_par_overlap in pick(
                                            tp > 1, ["none", "ring", "pipe"], ["none"]
                                        ):
                                            for weight_offload in pick(
