...
from calculon.util import arg_true_false_all, pick
...
class OptimalExecution(calculon.CommandLine):
    ...
    def create_parser(subparser):
        sp = subparser.add_parser(
            OptimalExecution.NAME,
            aliases=OptimalExecution.ALIASES,
        ...
        sp.add_argument(
            "application", type=str, help="File path to application configuration"
        )
        sp.add_argument("num_procs", type=int, help="Number of processors in execution")
        sp.add_argument(
            "max_batch_size",
            type=int,
        ...
        sp.add_argument(
            "datatype",
            type=str,
            choices=System.supported_datatypes(),
            help="The datatype to use",
        )
        sp.add_argument("system", type=str, help="File path to system configuration")
        sp.add_argument(
            "output",
            type=str,
        ...
        sp.add_argument(
            "-t", "--top-n", type=int, default=1, help="Number of best outputs"
        )
        sp.add_argument(
            "-l",
            "--layers",
        ...
        sp.add_argument(
            "-f",
            "--fused_activation",
            type=arg_true_false_all,
            default="true",
            help="Mode of fused activation",
        )
        sp.add_argument(
            "--no-tp-overlap", action="store_true", help="Don't allow TP overlap"
        )
        sp.add_argument(
            "--no-dp-overlap", action="store_true", help="Don't allow DP overlap"
        )
    ...
    def run_command(logger, args):
        ...
        for tp in Llm.get_all_tensor_parallelisms(
            ...
            for pp in Llm.get_all_pipeline_parallelisms(
                ...
                for ppint in Llm.get_valid_pipeline_interleavings(app.num_blocks, pp):
                    batch_size = OptimalExecution.get_batch_size(
                        dp, args.max_batch_size
                    )
                            ...
                            for tensor_par_comm_type in ["ar", "p2p_rs_ag", "rs_ag"]:
                                params.append(
                                    (
                                        ...
                                        args.mbs_break,
                                        not args.no_tp_overlap,
                                        not args.no_dp_overlap,
                                    )
                                )
        ...
        for index, run in enumerate(best):
            ...
            output[index] = {"execution": execution, "stats": stats}
        ...
        if calculon.io.is_json_extension(args.output):
            ...
            with opener(args.output, "wb") as fd:
                fd.write(
                    bytes(f',{",".join(exe_keys)},{",".join(stats_keys)}\n', "utf-8")
                )
                for index in sorted(output.keys()):
                    fd.write(bytes(f"{index}", "utf-8"))
                    for exe_key in exe_keys:
                        fd.write(
                            bytes(f',{output[index]["execution"][exe_key]}', "utf-8")
                        )
                    for stats_key in stats_keys:
                        fd.write(
                            bytes(f',{output[index]["stats"][stats_key]}', "utf-8")
                        )
                    fd.write(bytes("\n", "utf-8"))
        else:
    ...
    def search(
        ...
        for seq_par_ag_redo in pick(can_redo, [True, False], [False]):
            for data_par_overlap in pick(
                dp > 1 and allow_dp_overlap, [True, False], [False]
            ):
                for tensor_par_overlap in pick(
                    tp > 1 and allow_tp_overlap, ["none", "ring", "pipe"], ["none"]
                ):
                    for weight_offload in pick(has_mem2, [True, False], [False]):
                        ...
                        for activations_offload in activations_offloads:
                            for optimizer_offload in pick(
                                has_mem2, [True, False], [False]
                            ):
                                for fused_act in fused_acts:
                                    for (
                                        microbatch_size
                                    ) in Llm.get_valid_microbatch_sizes(
                                        app.seq_size, tp, dp, batch_size, pp
                                    ):
                                        mbs_break_good = good_exe_count
                                        for tn in pick(tp > 1, range(num_nets), [0]):
                                            for pn in pick(
                                                pp > 1, range(num_nets), [0]
                                            ):
                                                for dn in pick(
                                                    dp > 1, range(num_nets), [0]
                                                ):
                                                    exe_count += 1
                                                    exe_json = {
                                                        "num_procs": num_procs,
                                                    ...
                                                    if not debug:
                                                        try:
                                                            ...
                                                            model.compile(
                                                                syst,
                                                                Llm.Execution.from_json(
                                                                    exe_json
                                                                ),
                                                            )
                                                            model.run(syst)
                                                            stats = (
                                                                model.get_stats_json(
                                                                    layers
                                                                )
                                                            )
                                                            good_exe_count += 1
                                                            ...
                                                            best = OptimalExecution.update_list(
                                                                best, curr, top_n
                                                            )
                                        ...
                                        if (
                                            mbs_break
                                            and good_exe_count == mbs_break_good
                                        ):
                                            break
