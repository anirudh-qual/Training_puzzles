...
class Validation(calculon.CommandLine):
    ...
    def create_parser(subparser):
        ...
        sp.add_argument("-b", "--base_dir", default=".", help="Base directory")
        sp.add_argument(
            "-v",
            "--verbose",
            action="store_true",
    ...
    def run_command(logger, args):
        funcs = [Validation.seqsel_fig1, Validation.seqsel_fig7, Validation.seqsel_tab5]
        for func in funcs:
            if args.verbose:
    ...
    def seqsel_fig1(logger, args):
        ...
        def get_files(model, mode):
            ...
            exe = os.path.join(
                args.base_dir, "validation", "seqsel", "fig1", f"{model}_{mode}.json"
            )
            return app, exe
    ...
    def seqsel_fig7(logger, args):
        ...
        def get_files(model, mode):
            ...
            app = os.path.join(args.base_dir, "models", f"{model}.json")
            exe = os.path.join(
                args.base_dir, "validation", "seqsel", "fig7", f"{model}_{mode}.json"
            )
            return app, exe
    ...
    def seqsel_tab5(logger, args):
        ...
        def get_files(model, mode):
            ...
            app = os.path.join(args.base_dir, "models", f"{model}.json")
            exe = os.path.join(
                args.base_dir, "validation", "seqsel", "tab5", f"{model}_{mode}.json"
            )
            return app, exe
