...
class Llm:
    ...
    class Execution:
        ...
        def __init__(
            ...
            assert (
                self.num_procs == self.tensor_par * self.pipeline_par * self.data_par
            ), "tensor * pipeline * data parallelism != num_procs"
            ...
            if self.seq_par_ag_redo:
                ...
                assert (
                    self.activation_recompute != "full"
                ), "We assume no extra AG with full recompute"
            self._pipeline_par_rs_ag = self.tensor_par_comm_type in [
                "p2p_rs_ag",
                "rs_ag",
            ]
            ...
            for di in range(self.data_par):
                for pi in range(self.pipeline_par):
                    for ti in range(self.tensor_par):
                        nid = (
                            di * self.tensor_par * self.pipeline_par
                            + pi * self.tensor_par
                            + ti
                        )
                        peers[nid] = {}
                        ...
                        if self.data_par > 1:
                            ...
                            for di2 in range(self.data_par):
                                pid = (
                                    di2 * self.tensor_par * self.pipeline_par
                                    + pi * self.tensor_par
                                    + ti
                                )
    ...
    def get_all_pipeline_parallelisms(num_procs, tensor_par, num_blocks):
        ...
        for cand in Llm._factors(max_pp):
            if num_procs % (tensor_par * cand) == 0 and num_blocks % cand == 0:
                yield cand

    @staticmethod
    def get_data_parallelism(num_procs, tensor_par, pipeline_par):
        assert (
            num_procs % (tensor_par * pipeline_par) == 0
    ...
    def _build_attn_block(self):
        ...
        self._llm_block.append(
            Fork(
                "AttnBlock_Fork",
                self.sys,
                pick(
                    self.exe._sequence_par,
    ...
    def compile(self, sys, exe):
        ...
        if self.exe.pipeline_interleaving > self._blocks_per_proc:
            raise self.Error(
                "Pipeline interleaving must be less than or equal to "
                "the number of blocks per processor"
            )
        if self._blocks_per_proc % self.exe.pipeline_interleaving != 0:
            raise self.Error(
                "Pipeline interleaving must be a factor value of the "
        ...
        # performed
        if (
            self.exe.weight_offload
            or self.exe.activations_offload
            or self.exe.optimizer_offload
        ) and (self._blocks_per_proc <= 2):
            raise self.Error(
                "Offloading requires each processor to handle at least" " 3 blocks"
            )
        ...
        # block that is repeated N-1 times and followed by 1 edge block.
        # Recommunication time is the same in both base and edge blocks.
        self._blocks_per_chunk = self._blocks_per_proc // self.exe.pipeline_interleaving
        assert (
            self._blocks_per_proc % self._blocks_per_chunk == 0
        ...
        assert (
            self._chunks_per_proc == self.exe.pipeline_interleaving
    ...
    def _check_network_assignments(self):
        ...
        for tier_used, tier_size, tier in zip(used, size, range(self.sys.num_networks)):
            if tier_used:
                if tier_size > self.sys.get_network(tier).size:
                    raise self.Error(f"Network tier{tier} isn't big enough")
                if (
                    self.sys.get_network(tier).must_be_filled
                    and self.sys.get_network(tier).size % tier_size != 0
                ):
    ...
    def _compute_block_stats(self):
        ...
        for layer in self._llm_block:
            ...
            self._baseblock_fw_tp_time_exposed += layer.get_exposed_net_time(
                "fw", baseblock=True
            )
            self._edgeblock_fw_tp_time_exposed += layer.get_exposed_net_time(
                "fw", baseblock=False
            )
            self._tp_bw_overlap_req = max(
                self._tp_bw_overlap_req,
            ...
            if self.exe.training:
                ...
                if layer.get_recomm_flag():
                    self._baseblock_recomm_size += layer.get_comm_bytes(
                        "wgrad", baseblock=True
                    )
                    self._edgeblock_recomm_size += layer.get_comm_bytes(
                        "wgrad", baseblock=False
                    )
                    self._baseblock_recomm_time += layer.compute_net_time(
                        "wgrad", baseblock=True
                    )
                    self._edgeblock_recomm_time += layer.compute_net_time(
                        "wgrad", baseblock=False
                    )
                ...
                self._baseblock_agrad_tp_size += layer.get_comm_bytes(
                    "agrad", baseblock=True
                )
                self._edgeblock_agrad_tp_size += layer.get_comm_bytes(
                    "agrad", baseblock=False
                )
                self._baseblock_agrad_tp_time += layer.compute_net_time(
                    "agrad", baseblock=True
                )
                self._edgeblock_agrad_tp_time += layer.compute_net_time(
                    "agrad", baseblock=False
                )
            ...
            self.log.debug(
                "%s %s %s", layer.name, "Recomm flag:", str(layer.get_recomm_flag())
            )
            self.log.debug(
                "%s %s %s",
                layer.name,
            ...
            self.log.debug(
                "%s %s %s", layer.name, "Stores output:", str(layer.stores_output())
            )
            self.log.debug(
                "%s %s %s",
                layer.name,
            ...
            self.log.debug(
                "%s %s %s",
                layer.name,
                "FW baseblock comm tile size:",
                human_format(layer.get_comm_tile("fw", baseblock=True), "bytes"),
            )
            self.log.debug(
                "%s %s %s",
                layer.name,
                "FW edgeblock comm tile size:",
                human_format(layer.get_comm_tile("fw", baseblock=False), "bytes"),
            )
            self.log.debug(
                "%s %s %s",
                layer.name,
                "FW baseblock comm size:",
                human_format(layer.get_comm_bytes("fw", baseblock=True), "bytes"),
            )
            self.log.debug(
                "%s %s %s",
                layer.name,
                "FW edgeblock comm size:",
                human_format(layer.get_comm_bytes("fw", baseblock=False), "bytes"),
            )
            self.log.debug(
                "%s %s %.3e",
                layer.name,
            ...
            self.log.debug(
                ...
                "BW flops:",
                human_format(
                    layer.get_agrad_flops() + layer.get_wgrad_flops(), "flops"
                ),
            )
            self.log.debug(
                "%s %s %s",
            ...
            self.log.debug(
                ...
                "BW mem:",
                human_format(
                    layer.get_agrad_mem_accessed() + layer.get_wgrad_mem_accessed(),
                    "bytes",
                ),
            )
            self.log.debug(
                "%s %s %s",
                layer.name,
                "BW baseblock comm tile size:",
                human_format(layer.get_comm_tile("agrad", baseblock=True), "bytes"),
            )
            self.log.debug(
                "%s %s %s",
                layer.name,
                "BW edgeblock comm tile size:",
                human_format(layer.get_comm_tile("agrad", baseblock=False), "bytes"),
            )
            self.log.debug(
                "%s %s %s",
                layer.name,
                "BW baseblock comm size:",
                human_format(layer.get_comm_bytes("agrad", baseblock=True), "bytes"),
            )
            self.log.debug(
                "%s %s %s",
                layer.name,
                "BW edgeblock comm size:",
                human_format(layer.get_comm_bytes("agrad", baseblock=False), "bytes"),
            )
            self.log.debug(
                "%s %s %.3e",
                layer.name,
            ...
            self.log.debug(
                "%s %s %s",
                layer.name,
                "Recomm baseblock comm tile size:",
                human_format(layer.get_comm_tile("wgrad", baseblock=True), "bytes"),
            )
            self.log.debug(
                "%s %s %s",
                layer.name,
                "Recomm edgeblock comm tile size:",
                human_format(layer.get_comm_tile("wgrad", baseblock=False), "bytes"),
            )
            self.log.debug(
                "%s %s %s",
                layer.name,
                "Recomm baseblock comm size:",
                human_format(layer.get_comm_bytes("wgrad", baseblock=True), "bytes"),
            )
            self.log.debug(
                "%s %s %s",
                layer.name,
                "Recomm edgeblock comm size:",
                human_format(layer.get_comm_bytes("wgrad", baseblock=False), "bytes"),
            )
            self.log.debug(
                "%s %s %.3e",
                layer.name,
                "Recomm net link time:",
                layer.compute_net_time("wgrad"),
            )
            self.log.debug(
                "%s %s %.3e",
                layer.name,
                "Recomm net exposed time:",
            ...
            self.log.debug(
                "%s %s %.3e", layer.name, "Recompute:", layer.get_recompute_flag()
            )
            self.log.debug(
                "%s %s %s",
                layer.name,
                "Recompute mem saving:",
                human_format(layer.stores_output() * layer.get_output(), "bytes"),
            )
            self.log.debug(
                "%s %s %s",
                layer.name,
        ...
        self.log.debug(
            "%s %s",
            "TP comm FW baseblock size:",
            human_format(self._baseblock_fw_tp_size, "bytes"),
        )
        self.log.debug(
            "%s %s",
            "TP comm FW edgeblock size:",
            human_format(self._edgeblock_fw_tp_size, "bytes"),
        )
        self.log.debug(
            "%s %s", "PP comm FW size:", human_format(self._block_fw_pp_size, "bytes")
        )
        self.log.debug(
            "%s %s",
            "TP comm BW baseblock size:",
            human_format(self._baseblock_agrad_tp_size, "bytes"),
        )
        self.log.debug(
            "%s %s",
            "TP comm BW edgeblock size:",
            human_format(self._edgeblock_agrad_tp_size, "bytes"),
        )
        self.log.debug(
            "%s %s", "PP comm BW size:", human_format(self._block_bw_pp_size, "bytes")
        )
        self.log.debug(
            "%s %s",
            "TP recomm baseblock size:",
            human_format(self._baseblock_recomm_size, "bytes"),
        )
        self.log.debug(
            "%s %s",
            "TP recomm edgeblock size:",
            human_format(self._edgeblock_recomm_size, "bytes"),
        )
        self.log.debug(
            "%s %s",
            "TP comm required bandwidth for tiled overlap:",
    ...
    def _compute_batch_stats(self):
        ...
        self._optim_mem_accessed = (
            self._blocks_per_proc * self._block_optim_mem_accessed
        )
        ...
        tp_fw_comm_time = (
            self.exe._num_microbatches
            * self._chunks_per_proc
            * (
        ...
        self.log.debug(
            "%s %s", "TP comm baseblock FW time:", self._baseblock_fw_tp_time
        )
        self.log.debug(
            "%s %s", "TP comm edgeblock FW time:", self._edgeblock_fw_tp_time
        )
        self.log.debug("%s %s", "TP comm FW time:", tp_fw_comm_time)
        self.log.debug(
            "%s %s",
            "TP comm baseblock FW exposed time:",
            self._baseblock_fw_tp_time_exposed,
        )
        self.log.debug(
            "%s %s",
            "TP comm edgeblock FW exposed time:",
            self._edgeblock_fw_tp_time_exposed,
        )
        self.log.debug("%s %s", "TP comm FW exposed time:", tp_fw_comm_time_exposed)
        self.log.debug(
            "%s %s", "TP comm baseblock BW time:", self._baseblock_agrad_tp_time
        )
        self.log.debug(
            "%s %s", "TP comm edgeblock BW time:", self._edgeblock_agrad_tp_time
        )
        self.log.debug("%s %s", "TP comm BW time:", tp_bw_comm_time)
        self.log.debug(
            "%s %s",
            "TP comm baseblock BW exposed time:",
            self._baseblock_agrad_tp_time_exposed,
        )
        self.log.debug(
            "%s %s",
            "TP comm edgeblock BW exposed time:",
            self._edgeblock_agrad_tp_time_exposed,
        )
        ...
        self._edgeblock_fw_time_no_offload = (
            self._block_fw_time + self._edgeblock_fw_tp_time_exposed + chunk_fw_pp_time
        )
        self._baseblock_fw_offload_overhead = max(
            0,
            self.get_fw_offload_time()
            + self._block_fw_mem_time
            - self._baseblock_fw_time_no_offload,
        )
        ...
        chunk_fw_time = (self._baseblocks_per_chunk * self._baseblock_fw_time) + (
            self._edgeblocks_per_chunk * self._edgeblock_fw_time
        )
        chunk_bw_time = (self._baseblocks_per_chunk * self._baseblock_bw_time) + (
        ...
        self.log.debug(
            "%s %s",
            "Baseblock REcomm exposed time:",
            self._baseblock_recomm_time_exposed,
        )
        self.log.debug(
            "%s %s",
            "Edgeblock REcomm exposed time:",
            self._edgeblock_recomm_time_exposed,
        )
        ...
        self.log.debug(
            "DP block comm size: %s", human_format(self._block_dp_size, "bytes")
        )
            ...
            if self.exe.data_par_overlap:
                ...
                if self._dp_net == self._pp_net:
                    if self.exe._num_microbatches % self.exe.pipeline_par != 0:
                        ...
                        num_overlapped_pp = min(
                            chunk_dp_time // chunk_bw_time, self.exe.pipeline_par
                        )
                else:
                ...
                overlap_inflection = (
                    chunk_dp_time
                    - (overlap_window - num_overlapped_pp * chunk_bw_pp_time)
                    + overlap_compute * self._dp_net.processor_usage
                )
                if overlap_inflection > 0:
                    # Tcomm is larger than compute, excess is exposed
                    overlappable_chunks_exposed_time = (
                ...
                # but the last one.
                chunk_overlap_time = (
                    overlap_window + overlap_compute * self._dp_net.processor_usage
                )
                if self._dp_net == self._pp_net:
                ...
                if chunk_overlap_time > 0:
                    ...
                    if self.exe.optimizer_sharding:
                        ...
                        self._dp_bw_overlap_req_chunk *= self._dp_net._ops[
                            "all_reduce"
                        ].scalar
                else:
                ...
                if tail_overlap_time > 0:
                    self._dp_bw_overlap_req_tail = (
                        self._blocks_per_chunk * self._block_dp_size / tail_overlap_time
                    )
                    if self.exe.optimizer_sharding:
                        self._dp_bw_overlap_req_tail *= (
                            self._dp_net._ops["reduce_scatter"].scalar
                        ...
                        self._dp_bw_overlap_req_tail *= self._dp_net._ops[
                            "all_reduce"
                        ].scalar
                else:
                ...
                self.log.debug(
                    "Chunk exposed time: %.3e",
                    max(
                        0,
                        chunk_dp_time
                        + num_overlapped_pp * chunk_bw_pp_time
                        - overlap_window,
                    ),
                )
        ...
        self.log.debug(
            "DP comm required bandwidth for overlapped chunks: %s",
            human_format(self._dp_bw_overlap_req_chunk, "bandwidth"),
        )
        self.log.debug(
            "DP comm required bandwidth for the last chunk: %s",
        ...
        if self.exe.training:
            ...
            if self.exe.activation_recompute == "full":
                ...
                self._act_checkpoint_size = (
                    self._blocks_per_proc * self._block_act_checkpoint_size
                )
        ...
        # We should keep non-sharded weight grad for a current block for all_reduce
        ...
        if self.exe.training:
            ...
            self._optimizer_space = self._block_optimizer_space * self._blocks_per_proc
        else:
    ...
    def get_useful_flops(self):
        total_flops = sum([block.get_fw_flops() for block in self._llm_block])
        if self.exe.training:
            total_flops += sum(
                [
                    block.get_agrad_flops()
                    + block.get_wgrad_flops()
                    + block.get_optim_step_flops()
    ...
    def get_compute_efficiency(self):
        ...
        compute_time = (
            self.get_fw_time() + self.get_bw_time() + self.get_optim_step_time()
        )
        perfect_time = (
            self._blocks_per_proc
            * self.exe._num_microbatches
            * total_flops
            / self.sys.matrix.flops(self.exe.datatype)
        )
    ...
    def get_offload_mem_bw_req(self):
        ...
        if self.exe.training:
            ...
            req_bw = max(
                self._get_fw_offload_size() / fw_offload_time,
